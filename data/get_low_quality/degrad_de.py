...
def DE_HOLE(img, h, w, region_mask, center=None, diameter=None, d_min=0.4, d_max=0.7):
    '''
    ...
    diameter_circle = random.randint(int(d_min * w), int(d_max * w))
...
def DE_ILLUMINATION(img, region_mask, h=512, w=512, brightness=0.3, contrast=0.4, saturation=0.4, d_min=0.4, d_max=0.7):
    # TODO：修改
    img, color_params = DE_COLOR(img, brightness, contrast, saturation)
    img, halo_params = DE_HALO(img, h, w, color_params['brightness_factor'])
    img, hole_params = DE_HOLE(img, h, w, region_mask, d_min, d_max)
...
def DE_SPOT(img, h, w, center=None, radius=None, s_max_num=10):
    '''
...
def DE_BLUR_ODIR(img, h, w, center=None, radius=None, ratio=1):
    '''
    ...
    sigma = 5+(15-5) * random.random() * ratio + random.randint(5,30)
...
def DE_BLUR(img, h, w, center=None, radius=None, ratio=1):
    '''
...
def DE_process(img, mask, h, w, de_type):
    ...
    params = {}
    if de_type == '001':
        img, illum_params = DE_ILLUMINATION(img, mask, h, w, brightness, contrast, saturation, d_min, d_max)
        params['illumination'] = illum_params
    elif de_type == '010':
        img = transform(img)
        img = img.numpy()
        img, spot_params = DE_SPOT(img, h, w, s_max_num)
        ...
        img = transform(img)
        img = img.numpy()
        img, blur_params = DE_BLUR(img, h, w, ratio)
...
def DE_ODIR_process(img, mask, h, w, de_type):
    brightness = 0.8
    contrast = 0.8
    saturation = 0.8
    s_max_num = 20
    ...
    if de_type == '001':
        ...
        img = transform(img)
        img = img.numpy()
        img, blur_params = DE_BLUR_ODIR(img, h, w, ratio)
        params['blur'] = blur_params
    elif de_type == '101':
        img, illum_params = DE_ILLUMINATION(img, mask, h, w, brightness, contrast, saturation, d_min, d_max)
        params['illumination'] = illum_params
